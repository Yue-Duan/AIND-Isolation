...
def custom_score_2(game, player):
    ...
    if game.is_winner(player) or game.is_loser(player):
        return game.utility(player)
    moves = len(game.get_legal_moves())
    return float(random.randrange(-100, 100))
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        # player = game.active_player
        for m in game.get_legal_moves():
            v = self._min_val(game.forecast_move(m), 1, alpha, beta)
            if v > alpha:
                alpha = v
                best_move = m
            alpha = max(alpha, v)
    ...
    def _max_val(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        ...
        v = float("-inf")
        for m in legal_moves:
            v = max(v, self._min_val(game.forecast_move(m), depth+1, alpha, beta))
            if v >= beta:
                return v
            alpha = max(alpha, v)
        return v
    ...
    def _min_val(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        ...
        v = float("inf")
        for m in legal_moves:
            v = min(v, self._max_val(game.forecast_move(m), depth+1, alpha, beta))
            if v <= alpha:
                return v
            beta = min(beta, v)
        return v
